...
def clean_data_claim(initial_dataset: pd.DataFrame, new_person: pd.DataFrame):
    return initial_dataset

def retrained_model_claim(cleaned_dataset: pd.DataFrame):
...
def predict_claim(model, new_patient_data:pd.DataFrame):
    ...
    return pd.DataFrame({'claim': [new_patient_claim]})